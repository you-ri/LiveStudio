...
- Translations are registered in edit mode too. The remote-control server answers a remote app from a stopped editor, and `RuntimeInitializeOnLoadMethod` only fires on play, so until then every Social label and help text reached the remote app as its raw key.
