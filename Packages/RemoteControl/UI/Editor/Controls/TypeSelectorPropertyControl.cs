...
{
    /// <summary>
    /// TypeSelector属性付きSerializeReferenceフィールド用コントロール。
    /// ドロップダウンで派生型を切り替え、選択した型のプロパティをフラットに表示する。
    /// </summary>
    public class TypeSelectorPropertyControl : IPropertyControl
    {
        private const float kPropertyNameWidth = 160f;

        /// <summary>
        /// ネストプロパティの配置先。設定されている場合、型切り替え時にここへ再構築する。
        /// 未設定の場合はtype-selector-container内に配置する。
        /// </summary>
        public VisualElement nestedPropsTarget { get; set; }

        public VisualElement CreateControl(PropertyControlContext ctx)
        {
            var typeSelector = ctx.propType.controlAttribute as TypeSelectorAttribute;
            if (typeSelector == null || typeSelector.options == null || typeSelector.options.Length == 0)
                return new ReadOnlyLabelControl().CreateControl(ctx);

            // 縦並びコンテナ（ドロップダウン + ネストプロパティ）
            var container = new VisualElement();
            container.name = "type-selector-container";
            ...
            var options = typeSelector.options.ToList();
            ...
            if (!ctx.isReadOnly)
            {
                ...
                popup.RegisterValueChangedCallback(evt =>
                {
                    ...
                    var oldProps = propsParent.Q("type-selector-props");
                    if (oldProps != null)
                    ...
                    newProps.name = "type-selector-props";
        ...
        {
            var popup = control.Q<PopupField<string>>("type-selector-popup");
