...
{
    /// <summary>
    /// Registration information for LiveUnityObject types
    /// </summary>
    public class TypeRegistration
    {
        public string displayName;
        public Type componentType;
        public Func<UnityEngine.Object, LiveUnityObjectBase> factory;

        public TypeRegistration(string displayName, Type componentType, Func<UnityEngine.Object, LiveUnityObjectBase> factory)
        {
    ...
    /// <summary>
    /// Factory class for creating LiveUnityObjectBase instances from Unity components.
    /// Supports registration-based type management.
    /// </summary>
    public static class LiveUnityObjectFactory
    {
        ...
        /// <summary>
        /// Registers a type for LiveUnityObject creation
        /// </summary>
        /// <typeparam name="T">The Unity component type</typeparam>
        /// <param name="displayName">Display name for UI</param>
        /// <param name="factory">Factory function to create LiveUnityObjectBase</param>
        public static void Register<T>(string displayName, Func<T, LiveUnityObjectBase> factory) where T : UnityEngine.Object
        {
            var componentType = typeof(T);

            // Wrap the factory to handle casting
            Func<UnityEngine.Object, LiveUnityObjectBase> wrappedFactory = (obj) =>
            {
                if (obj == null)
                {
                    return factory(null);
        ...
        /// <summary>
        /// Creates an LiveUnityObjectBase from a Unity component.
        ...
        /// <returns>An LiveUnityObjectBase instance</returns>
        ...
        public static LiveUnityObjectBase Create<T>(T component) where T : UnityEngine.Object
        {
            if (component == null)
            {
            ...
            throw new NotSupportedException($"[RemoteControl] Type {typeof(T).Name} is not registered for LiveUnityObject creation.");
        }

        /// <summary>
        /// Tries to create an LiveUnityObjectBase from a Unity component.
        /// Returns false if the type is not registered.
        /// </summary>
        /// <typeparam name="T">The type of Unity component</typeparam>
        /// <param name="component">The Unity component to wrap</param>
        /// <param name="result">The created LiveUnityObjectBase instance, or null if creation failed</param>
        /// <returns>True if creation succeeded, false otherwise</returns>
        public static bool TryCreate<T>(T component, out LiveUnityObjectBase result) where T : UnityEngine.Object
        {
            result = null;
            ...
            if (_typeRegistry.TryGetValue(componentType, out var registration))
            {
                ...
                {
                    Debug.LogError($"[RemoteControl] Failed to create LiveUnityObject for type {componentType.Name}: {e.Message}");
                    return false;
                }
            }
        ...
        private static void _EnsureAutoRegistered()
        {
            ...
            _AutoRegisterDerivedTypes(typeof(LiveUnityObjectProxy<,>));
            _AutoRegisterDerivedTypes(typeof(LiveUnityObjectReference<,>));
        }
        ...
        private static void _AutoRegisterDerivedTypes(Type openGenericBase)
        {
            foreach (var assembly in AssemblyUtility.GetLoadedAssemblies())
            {
                ...
                foreach (var type in types)
                {
                    ...
                    var classAttr = type.GetCustomAttribute<LiveClassAttribute>();
                    ...
                    {
                        return (LiveUnityObjectBase)ctor.Invoke(new object[] { obj });
                    });

                    // componentType 単位の default 自動ラップ先は先勝ち (例: GameObject → LiveGameObject)。
                    // 同 componentType で displayName が異なる Proxy (例: LiveGameObjectWithTransform) は
