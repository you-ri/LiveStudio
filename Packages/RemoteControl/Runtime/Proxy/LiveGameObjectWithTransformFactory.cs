...
{
    [Serializable]
    [MovedFrom(false, null, null, "ExposedGameObjectWithTransformFactory")]
    public class LiveGameObjectWithTransformFactory : ILiveObjectFactory
    {
        [SerializeField]
        ...
        public ILiveObject Create()
        {
            if (prefab == null)
            {
                Debug.LogError("[RemoteControl] LiveGameObjectWithTransformFactory.Create: prefab is null.");
                return null;
            }
            ...
            if (instance == null)
            {
                Debug.LogError($"[RemoteControl] LiveGameObjectWithTransformFactory.Create: failed to instantiate prefab '{prefab.name}'.");
                return null;
            }

            var exposed = new LiveGameObjectWithTransform(instance);
        ...
        public void RegisterPrefabs()
        {
            ...
            {
                Debug.LogWarning($"[RemoteControl] LiveGameObjectWithTransformFactory.RegisterPrefabs: prefab '{prefab.name}' has no guid. Open the containing asset in Inspector to trigger OnValidate.");
                return;
            }
