...
{
    [Serializable]
    public class ExposedGameObjectFactory : IExposedObjectFactory
    {
        [SerializeField]
        ...
        public IExposedObject Create()
        {
            if (prefab == null)
            {
                Debug.LogError("[RemoteControl] ExposedGameObjectFactory.Create: prefab is null.");
                return null;
            }
            ...
            if (instance == null)
            {
                Debug.LogError($"[RemoteControl] ExposedGameObjectFactory.Create: failed to instantiate prefab '{prefab.name}'.");
                return null;
            }

            var exposed = new ExposedGameObject(instance);
        ...
        public void RegisterPrefabs()
        {
            ...
            {
                Debug.LogWarning($"[RemoteControl] ExposedGameObjectFactory.RegisterPrefabs: prefab '{prefab.name}' has no guid. Open the containing asset in Inspector to trigger OnValidate.");
                return;
            }
