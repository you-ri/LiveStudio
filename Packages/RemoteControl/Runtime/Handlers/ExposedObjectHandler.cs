...
namespace Lilium.RemoteControl
{
    ...
    public class ExposedObjectHandler : BaseRemoteControlApiHandler
    {
        ...
        protected override async Task HandlePostRequest(HttpListenerContext context)
        {
            ...
            // エクスポート
            else if (path.Equals("/exposed/export", System.StringComparison.OrdinalIgnoreCase))
            {
                await HandleExport(context);
                return;
            }
            // インポート
            else if (path.Equals("/exposed/import", System.StringComparison.OrdinalIgnoreCase))
            {
