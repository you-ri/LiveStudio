...
namespace Lilium.RemoteControl
{
    ...
    /// LiveClass の cctor を巻き込まないよう、別クラスに分離している。
    /// (LiveClass.RegisterDeclarationOrder を直接呼ぶと、登録ループの途中で
    /// LiveClass.cctor が起動して _RegisterAllTypesFromAttributes が走り、
    /// まだ登録されていない型に対して "宣言順未登録" 警告が誤発火する。)
    /// </summary>
    public static class LiveClassDeclarationOrderTable
    {
        // key = [LiveClass] が付いた Type、value = メンバー名 → 宣言順 index。
