...
{
    /// <summary>
    /// LiveObjectのデフォルト値をJObjectとして一元管理し、dirty判定・revert機能を提供する。
    ///
    /// キーはtargetオブジェクト（インスタンス型）またはLiveClass.type（static型）を使用し、
    /// LiveObjectインスタンスへの辞書キー依存を排除している。
    ///
    /// 2つのJObjectを管理:
    ...
    /// </summary>
    public static class LiveObjectDefaultRegistry
    {
        ...
        /// <summary>
        /// LiveObjectからディクショナリキーを取得する。
        /// インスタンス型: target（参照等価）
        /// static型: LiveClass.type（Type参照）
        /// </summary>
        private static object _GetKey(LiveObjectHandle obj)
        {
        ...
        /// <summary>
        /// LiveObjectの現在の状態をデフォルトJSONとしてキャプチャする。
        /// _serializationBaselineを設定し、_userChangeBaselineをリセットする。
        /// </summary>
        public static void CaptureDefaults(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
        ...
        /// </summary>
        public static void CaptureDefaultsPreservingOverrides(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
            var fresh = LivePropertySerializer.SerializeFullToJObject(obj, resolver, forPersistence: true);
        ...
        /// <summary>
        /// 指定LiveObjectのデフォルトを削除する。
        /// </summary>
        public static void Remove(LiveObjectHandle obj)
        {
            var key = _GetKey(obj);
        ...
        /// 未登録の場合はnull。
        /// </summary>
        public static JObject GetDefaults(LiveObjectHandle obj)
        {
            var key = _GetKey(obj);
        ...
        /// </summary>
        public static bool IsDirty(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
            var defaultJson = _GetUserChangeBaseline(key);
            if (defaultJson == null) return false;

            // LivePropertyRef は baseline に含まれないため、current 側でも除外して比較する。
            //
            ...
            var currentJson = LivePropertySerializer.SerializeFullToJObject(obj, resolver, forPersistence: true, skipPropertyRef: true);
            var diff = LivePropertySerializer.JsonDiff(defaultJson, currentJson);
            ...
            if (obj.propertyTypes != null)
            {
                ...
                {
                    if (propType != null && propType.isLivePropertyReference)
                    {
        ...
        /// </summary>
        public static bool IsPropertyDirty(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
        ...
        /// </summary>
        public static bool HasDirtyChildProperty(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
            ...
            {
                return LivePropertySerializer.JsonDiff(defaultRootToken, currentRootToken) != null;
            }
            ...
            return LivePropertySerializer.JsonDiff(defaultSubToken, currentSubToken) != null;
        }

        /// <summary>
        /// dirtyなルートプロパティ名のリストを返す。
        /// </summary>
        public static IReadOnlyCollection<string> GetDirtyProperties(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
            var defaultJson = _GetUserChangeBaseline(key);
            ...
            foreach (var propType in obj.propertyTypes)
            {
                ...
                // LivePropertyRef は自身の baseline を持たない。参照先の dirty を判定する。
        ...
        /// </summary>
        public static void ClearDirty(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
        ...
        /// </summary>
        public static void MarkClean(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
            ...
            _userChangeBaseline[key] = LivePropertySerializer.SerializeFullToJObject(
        ...
        /// </summary>
        public static void ClearPropertyDirty(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
            ...
            var value = LivePropertyUtility.GetValueRaw(obj.target, propertyType);
        ...
        /// </summary>
        public static bool Revert(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
            if (string.IsNullOrEmpty(propertyPath)) return false;
            var key = _GetKey(obj);
            ...
            {
                defaultValue = LivePropertySerializer.DeserializeUnityType(
                    resolver, defaultToken, valueType);
            }
        ...
        /// </summary>
        public static void EnsureDefaultsCaptured(LiveObjectHandle obj, ILiveObjectResolver resolver)
        {
            var key = _GetKey(obj);
            if (!_serializationBaseline.ContainsKey(key))
            {
        ...
        /// </summary>
        public static void EnsurePropertyDefaultCaptured(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
            if (string.IsNullOrEmpty(propertyPath)) return;
        ...
        /// </summary>
        public static JToken GetDefaultToken(LiveObjectHandle obj, string propertyPath)
        {
        ...
        /// </summary>
        private static JToken _SerializePropertyValue(LiveObjectHandle obj, string propertyPath, ILiveObjectResolver resolver)
        {
            var property = obj.FindProperty(propertyPath);
        ...
        /// </summary>
        private static JToken _SerializeRootPropertyValue(LiveObjectHandle obj, string rootName, ILiveObjectResolver resolver)
        {
            var propertyType = obj.targetType?.FindProperty(rootName);
        ...
        private static JToken _SerializeWithControlAttribute(
            ILiveObjectResolver resolver, object value, LivePropertyType propertyType)
        {
            ...
            return LivePropertySerializer.SerializeUnityType(resolver, value, propertyType.forceValue, forPersistence: true);
        }
