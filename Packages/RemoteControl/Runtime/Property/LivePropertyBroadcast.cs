...
{
    /// <summary>
    /// Publishes LiveObject property changes to connected remote apps.
    ///
    /// Publishing records the changed object's id in <see cref="LiveChangeLog"/>; it does not send
    ...
    /// </summary>
    public static class LivePropertyBroadcast
    {
        /// <summary>
        ...
        {
            if (!LiveObjectRegistry.TryFindByTarget(target, out var liveObj)) return;
            BroadcastProperty(liveObj, propertyPath);
        }
        ...
        public static void BroadcastProperty(UnityEngine.Object target, string propertyPath)
        {
            ...
            LiveChangeLog.Record(LiveObjectUtility.GetInstanceID(target).ToString());
        }

        /// <summary>
        /// Publishes a change to the given property of <paramref name="liveObj"/>.
        /// </summary>
        public static void BroadcastProperty(LiveObjectHandle liveObj, string propertyPath)
        {
            // An unregistered handle has no id to key the change on; the client could not address it.
            if (!liveObj.hasId) return;
            LiveChangeLog.Record(liveObj.id);
        }

        /// <summary>
        /// Publishes a change to a property of a static <see cref="LiveClass"/>. A static class has
        ...
        public static void BroadcastStaticProperty(System.Type staticType, string propertyPath)
        {
            ...
            var liveClass = LiveClass.Find(staticType);
            if (liveClass == null) return;
            // Skip types that were never registered — the client has no object under that id.
            if (LiveObjectRegistry.FindById(liveClass.typeName) == null) return;

            LiveChangeLog.Record(liveClass.typeName);
        }

        /// <summary>
        /// Publishes that the LiveClass / LiveEnum tables have been rebuilt. Remote apps refetch
        /// /live/types and /live/enums together so the two stay consistent.
        /// </summary>
        public static void BroadcastTypesUpdate()
        {
            LiveChangeLog.Record(LiveChangeLog.kTypesId);
        }
    }
}
