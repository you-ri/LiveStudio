...
namespace Lilium.RemoteControl
{
    ...
    /// <c>LiveGameObjectWithTransform</c>) re-resolves and re-attaches.
    ///
    /// GameObject 引数を採用する理由: TransformRef.ownerName は GameObject の name 文字列で
    /// 永続化されるため、broadcasting する側も「どの GameObject の配下が変わったか」を
    /// 直接渡すのが整合的。LiveObjectHandle インスタンスを経由する必要がなく、owner が
    /// LiveObjectHandle として登録されていないケース (例: MonoBehaviour が直接 LiveObjectHandle 化
