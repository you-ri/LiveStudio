...
- **`GET /live/asset/image?id=<key>` — the picture for an asset, alongside the name and type `/live/asset` already returns.** A client that can label an asset can now show it, without the host application having to serve an image route of its own. Where the bytes come from stays with whoever owns the assets: `AssetRegistry.SetThumbnailProvider` is the injected hook (the same shape as the existing name-fallback / catalog-prewarm / group-expander hooks), and the id is passed through untouched, so an application-specific reference works as well as a registry key. Without a provider — or for an asset that has no picture — the answer is 404, which clients already treat as "fall back to a placeholder". This replaces LiveStudio's `/live/avatar/image` **and** its `/live/snapshot/image`, both of which were an application serving pictures for things that are assets; moving the one endpoint here is a breaking wire change and ships without a compatibility shim.
