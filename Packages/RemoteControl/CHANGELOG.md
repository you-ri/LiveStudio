...
- **The type table says `"persistScope": "custom"` for a member the owner files itself.** It already said `"project"` for a member written to the project settings, and left the live scene unsaid as the default; a member saved by its owner (a deck) was indistinguishable from a scene one. A client that sorts objects by where they are saved -- the live scene page showing what the scene file holds, the project page what the machine holds -- needs the third answer, or an owner-saved member passes for a scene one. Nothing else in the response moves: the key appears only on members declared `PersistScope.Custom`.

- **A live scene is saved as `*.scene.json`, and a recorded take as `*.live.bin`.** `.live.json` and `.livedata` were both named after the system rather than after what the file holds, and `.livedata` was the odd one out in a project where every other compound suffix says what it is (`.set.lsb`, `.prop.lsb`, `.deck.json`). Neither format changed — only the name a new file is written under. Existing files keep opening: loading a live scene never looked at the extension in the first place (the content is read regardless), and `.live.json` stays accepted by `IsLiveSceneFile`, so a project full of them still lists and opens. Takes recorded as `.livedata` stay listed and playable in the LiveData Sequencer and on the recorder page, which now enumerate both suffixes. What moves is the "Save As" default (`Untitled.scene.json`), the extension auto-completed when the user types a bare name, and the name a new take is written under.

- **The event lane is called `Event`, not `Input`: `InputFrame` is `EventFrame`, `InputRecord` is `EventRecord`, `FrameLane.Input` is `FrameLane.Event`, and eleven more.** Breaking for anything naming those types; the fix is mechanical. "Input" had come to mean three different things — a `FrameSource` (something that supplies the frame), this lane, and LiveStudio's own `InputSource` for operations — and the first of those feeds the *state* lane as often as this one, since capture pose and time both arrive from a declared source. Explaining the design required the sentence "an input source puts this on the state lane, not the input lane", which is the tell that a word has stopped working. The second reason is that this lane never behaved like an input stream: a property write carries its own value, so replaying it needs no re-computation. It is a sparse log of things that happened, which is what an event is. `FrameSource` and `FrameGate` keep their names — a source of input is still a source of input. `InputSymbolTable` became `FrameSymbolTable` on the way past, since both lanes use it (state blocks intern their type names there too). **Recordings are unaffected** — entry and event kinds are written as numbers, so not one byte of the format moves.

- **The frame types moved to a new assembly, `Lilium.RemoteControl.Simulation`.** Breaking for any assembly that names a frame type: add the reference. The simulation holds the state the live data is defined over, and the view (GameObject, Animator, Transform) presents it; the one direction that must never exist is the simulation reading back out of the view, because a value read from an evaluated `Transform` is not reproducible from a recording. The boundary drawn here is the dependency direction — the new assembly does not reference `Lilium.RemoteControl`, so it cannot reach live objects, the registry or the bridges, and there is no way into the object graph from inside it. Engine references are kept: the native containers and the job system live in UnityEngine, and this is the layer meant to run on them. What lives in the new assembly: the frame structures, the state and structure blocks, and the recording *format* (`FrameRecord`, `FrameRecordReader`, `FrameRecordWriter`). What stays: `FrameGate` (it hooks the PlayerLoop), the bridges, the recording *drivers* (`FrameRecorder`, `FrameReplayer`, `FrameRecordPlayer`) and the whole exposure surface. Two rules stay conventions rather than compile errors — do not read the view's output from the simulation, and read time from the live data rather than from `UnityEngine.Time` or the wall clock.

- **`FrameLane` is in `Lilium.RemoteControl.Core` now, not with the frame types.** It appears on `[LiveField(lane: ...)]` and `[LiveClass]`, so leaving it in the new assembly would have forced every assembly that merely *declares* exposure to reference the simulation. Nothing to change for callers that already reference Core; the type and namespace (`Lilium.RemoteControl`) are unchanged.

- ⚠ **An assembly that declares `lane = FrameLane.State` must reference `Lilium.RemoteControl.Simulation`.** The generated state bridge names types that live there. The generator now checks and reports `LRC004` naming the type and the fix, and withholds the state block rather than letting the failure surface as `CS0246` inside generated code the author never wrote.

### Added

- **A replay can loop.** `FrameReplayer.loop` starts the take again when it reaches the end instead of running out, and `FrameRecorderController.loop` is the switch for it -- settable while a replay is running, not only before one, since this is what an operator reaches for on the last few frames. The rewind is a seek back to the first frame, so it restores the shape of the world from the keyframe there the way any other jump does: a take that spawned something on the way through does not accumulate a second copy of it on the next pass. A recording with no tail index cannot be sought, so it plays once and ends whatever the switch says -- the same limitation as scrubbing, and for the same reason. Nothing loops by default, and a replay that ends still detaches the source and raises `onSourceEnded` exactly as before.

- **Two analyzers keep the simulation assembly honest: `LRC010` (do not touch a view object) and `LRC011` (do not read a clock of your own).** They ship in the existing generator DLL and run only when compiling `Lilium.RemoteControl.Simulation`, so nothing else in a project is affected. `LRC010` triggers on anything deriving from `UnityEngine.Object` — a value read from an evaluated scene object cannot be reproduced from a recording — which leaves value types such as `Vector3`, `NativeArray` and job structs alone; `LRC011` covers `UnityEngine.Time`, `DateTime.Now`, `Stopwatch` and `TimeUtility`, because time that drives the simulation has to arrive on the frame. Both are warnings, so the one legitimate exception — `RealtimeFrameClock`, which is the door real time comes through — suppresses `LRC011` with a comment saying why. This is deliberately narrower than banning the engine outright: an earlier attempt to declare `noEngineReferences` on the assembly also took away the job system and the native containers, which is the layer's whole purpose, while still not covering `DateTime`.

- **A LiveData Sequencer window (`Window/Lilium Remote Control/LiveData Sequencer`) records a take and plays one back.** The counterpart of the LiveData Viewer, and built to look like it: a status bar carrying the position (click to switch between a frame count and a timecode, both counted from the start of the take rather than from the start of the run), a transport, and the takes on disk. Each take is listed with the frames and the running time it actually holds, read out of its tail index — a take that was cut short, or one from a build whose recording format has moved on, says so on its own row instead of being dropped from the list, because a take that is there but unreadable and a take that was never written look the same once it is missing. Selecting one and pressing play replays it into the running application; the slider scrubs it. A take can be removed from its own row, behind a confirmation naming the file — these live outside the Unity project, so the editor’s own trash does not reach them and the delete is final; the take being written cannot be removed at all, and one being replayed stops first, because a handle on a file refuses the delete outright rather than deferring it. The header opens the folder they are written to, creating it first, since it only comes into being once something has been recorded. It drives the `FrameRecorderController` in the scene and owns no recorder of its own, so a remote page and this window can be open at once — and a stopped editor, or a scene without a recorder, disables the controls and says which of the two it is.

- **`FrameRecorderController` records and replays live data as a scene component — moved here from `jp.lilium.livestudio`.** Everything it does is this package's: the gate, the recorder, the replayer, retaining the state and structure systems for the length of a take, and putting the inventory back on a supplied frame. Getting that sequence right is not something each application should have to rediscover. The one thing it does not know is where an application files its work, which is now asked of `recordingFolderProvider` (defaulting to a folder beside the player's own data), and `ExcludeControlObject(id)` is how whatever drives it keeps its own buttons out of the takes — without which a replay presses them again, starting a second recording or tearing down the replay that is running it. Breaking for source naming `Lilium.LiveStudio.FrameRecorderController`; nothing has shipped it, the exposed class name is still `FrameRecorder`, and the script GUID moved with the file so existing prefabs and scenes keep their reference.

- **Held replay and scrubbing on the controller: `replayPaused`, `replayFrameCount`, `replayIndex`, `SeekReplay(index)` and `recordingPath`.** `replayIndex` is a position within the take, not the recorded frame number — see the seek fix below for why those are not the same number offset by a constant.

- **A frame source can be held: `FrameReplayer.isPaused`.** A held replayer keeps supplying the frame it last played — the lanes are pointed at what that frame left, and nothing is advanced or applied again — so the gate goes on taking its frame from the recording. Detaching instead would hand the world back to the live producers and overwrite the frame being looked at, which is the opposite of a pause. Held before the first frame it still plays one, because the player's lanes are empty until then and supplying those blanks the world rather than holding it.
...
- **The LiveData Sequencer and the LiveData Viewer are translated (English, Japanese, Simplified Chinese), and so is anything else this package's editor windows write.** They read their text through the same `LocalizationSystem` the application and the remote app share, so a window follows the language chosen there rather than holding one of its own — and English is now the source the text is written in, which it was not before. The files are the editor's own and live outside `Resources`, under `Editor/Localization/RemoteControlEditorLocales/`, since a player build has no use for strings describing windows that exist only in the editor. A key missing from the active language falls back to English before it falls back to the key: a window nobody has translated yet is still readable, and one full of `LDS_` tokens is not. Supporting this, `LocalizationSystem` gained `Format(key, args)` (placeholders belong *inside* the translated text — word order is the first thing a translation changes), `TryTranslate(language, key, out text)` for callers choosing their own fallback, and `generation`, which moves whenever the active language or the table behind it does. A window that resolves a label once, when it builds the row it sits in, watches that to learn it has to ask again — including after entering play mode, which empties the table.

### Fixed

- **Seeking to a frame finds it by number instead of counting on from the first, so a scrub lands where it was asked to.** `FrameRecordReader.TrySeekFrame` treated the index as contiguous — frame `n` at position `n - first` — which is only true of a run that never misses a beat. Frame numbers come from the clock, so a run below rate skips them: a 2628 frame take recorded at sixty hertz was measured spanning 2805 numbers with 58 gaps, and seeking into it landed up to tens of frames from where it was asked for. Silently, because the frame it landed on is a perfectly good frame. The lookup is now a binary search over the index, reading each probe's boundary out of the file — about a dozen small reads, against the walk over every frame the index exists to avoid. `FrameRecordReader` and `FrameRecordPlayer` gained `FrameNumberAt(index)` and `IndexOfFrame(frameNumber)` so a caller can convert between a position in a recording and a frame number rather than assuming; `firstFrameNumber` is exposed too, but only as what the recording starts at. Nothing about the format moved, so existing recordings read as they did — they just seek correctly now.
...
- **The Transform manipulator is part of this package now — `POST/DELETE /live/manipulator/open`, `GET /live/manipulator/image`, `PUT /live/manipulator/camera` (moved from `jp.lilium.livestudio`, wire unchanged).** It only ever touched this package's concepts (live objects, `TransformValue`, property paths), so the session-holding offscreen camera, the PNG frame with its `X-Manipulator-State` pose header and the pose route all belong here, and any host application gets gizmo editing without carrying a handler of its own. An object whose edited transform is expressed relative to a dynamic parent (an avatar socket, say) implements the moved `IManipulatorTarget` to describe its own frame. This is deliberately a route, not an image member: a session is per-dialog server state, the frame must travel with the exact pose that rendered it, and pose writes must not run through the editor-write scope (a viewpoint is not scene state, so it must not dirty the scene or the undo stack).

- **Image members: a live object can serve a picture straight off the generic property surface.** Declare a getter-only `[LiveProperty, ImagePreview]` member of the new `LiveImageData` type (encoded bytes + MIME type) and no bespoke route is needed: a direct `GET /live/object/{id}/{path}` answers the raw bytes with the right Content-Type, while every JSON-shaped read — the whole-object serialization, single-property reads, `/live/batch` — folds the member to that very address **without invoking the getter**, so listing an object never renders its pictures. The remote app's existing ImagePreview control polls whatever address the value carries, which makes the declaration self-describing and the client generic. The getter runs on the main thread once per picture request and should render, encode and return without caching — the client's poll is the refresh loop; returning an invalid `LiveImageData` answers 404, the same "fall back to a placeholder" contract as the asset thumbnail route. Image members never report `changed` (a picture is derived state with no baseline, and comparing one would render a frame per poll) and are never persisted. `[ImagePreview]` on a string member keeps its older meaning — the value is an address served elsewhere and the getter is called normally. Supported on top-level members only; inside nested composites the value degrades to null, since there is no address to fold to.
