...
{
    /// <summary>
    /// Tests for the depth-limited serialization added for the shallow <c>/exposed/objects</c> REST read
    ...
    /// </summary>
    [TestFixture]
    public class ExposedNestedDepthTests
    {
        [ExposedClass("DepthLeaf")]
        ...
        [ExposedClass("DepthChild")]
        public class DepthChild
        {
            [ExposedField] public int childValue;
            [ExposedField] public DepthLeaf leaf;
        }

        [ExposedClass("DepthRoot")]
        public class DepthRoot
        {
            [ExposedField] public int rootValue;
            [ExposedField] public string rootName;
            [ExposedField] public DepthChild child;
            [ExposedField] public DepthChild[] children;
            [ExposedField] public int[] scalars;
        }
        ...
        public void SetUp()
        {
            ...
            ExposedClass.RegisterFromAttributes<DepthLeaf>();
            ExposedClass.RegisterFromAttributes<DepthChild>();
            ExposedClass.RegisterFromAttributes<DepthRoot>();
        }
        ...
        private ExposedObjectHandle MakeRootHandle(string id = "root-1")
        {
            return new ExposedObjectHandle(id, ExposedClass.Find(typeof(DepthRoot)), MakeRootTarget());
        }
        ...
        {
            var json = ExposedPropertySerializer.ToJson(MakeRootHandle(), _resolver, maxDepth: 1);
        ...
        public void UnboundedDepth_ExpandsFully_NoTruncationMarker()
        {
            ...
            var json = ExposedPropertySerializer.ToJson(MakeRootHandle(), _resolver);
            var obj = JObject.Parse(json);
        ...
        {
            var a = ExposedPropertySerializer.ToJson(MakeRootHandle("same"), _resolver);
            var b = ExposedPropertySerializer.ToJson(MakeRootHandle("same"), _resolver, maxDepth: int.MaxValue);
            Assert.AreEqual(a, b);
        }
        ...
        {
            var json = ExposedPropertySerializer.ToJson(MakeRootHandle(), _resolver, maxDepth: 2);
            var obj = JObject.Parse(json);
        ...
        public void RegisteredChild_EmitsRef_UnaffectedByDepth()
        {
            ...
            var childHandle = new ExposedObjectHandle("child-1", ExposedClass.Find(typeof(DepthChild)), childTarget);

            var rootTarget = MakeRootTarget();
            rootTarget.child = childTarget;
            var rootHandle = new ExposedObjectHandle("root-ref", ExposedClass.Find(typeof(DepthRoot)), rootTarget);

            var shallow = JObject.Parse(ExposedPropertySerializer.ToJson(rootHandle, _resolver, maxDepth: 1));
            var deep = JObject.Parse(ExposedPropertySerializer.ToJson(rootHandle, _resolver));
            ...
            // childHandle is retained by ExposedObjectRegistry for the resolver to find.
        ...
        {
            var shallow = ExposedPropertySerializer.ToJson(MakeRootHandle(), _resolver, maxDepth: 1);
            ...
            var freshHandle = new ExposedObjectHandle(
                "fresh", ExposedClass.Find(typeof(DepthRoot)), freshTarget);

            Assert.DoesNotThrow(() => ExposedPropertySerializer.FromJson(shallow, freshHandle, _resolver));
        ...
        {
            Assert.AreEqual(1, ExposedObjectHandler.ResolveNestedMaxDepth(new NameValueCollection()));
            Assert.AreEqual(1, ExposedObjectHandler.ResolveNestedMaxDepth(null));
        }

        [Test]
        public void ResolveNestedMaxDepth_TruthyValues_AreUnbounded()
        {
            Assert.AreEqual(int.MaxValue, ExposedObjectHandler.ResolveNestedMaxDepth(Query("nested", "true")));
            Assert.AreEqual(int.MaxValue, ExposedObjectHandler.ResolveNestedMaxDepth(Query("nested", "1")));
            Assert.AreEqual(int.MaxValue, ExposedObjectHandler.ResolveNestedMaxDepth(Query("nested", "")));
            // Bare `?nested` (no '=') lands under the null key in .NET's parser.
            Assert.AreEqual(int.MaxValue, ExposedObjectHandler.ResolveNestedMaxDepth(Query(null, "nested")));
        }

        [Test]
        public void ResolveNestedMaxDepth_FalseyValue_IsShallow()
        {
            Assert.AreEqual(1, ExposedObjectHandler.ResolveNestedMaxDepth(Query("nested", "false")));
            Assert.AreEqual(1, ExposedObjectHandler.ResolveNestedMaxDepth(Query("nested", "0")));
        }
    }
}
