...
namespace Lilium.RemoteControl.Tests
{
    ...
    /// `[LiveField, Hide][FormerlyNamedAs("X")]` Field "_X" is collapsed into
    ...
    public class ShadowFieldTests
    {
        ...
        /// </summary>
        [Serializable]
        [LiveClass("TestShadowFieldHost")]
        public class TestHost
        {
            [SerializeField, LiveField, Hide]
            [FormerlyNamedAs("value")]
        ...
        /// </summary>
        [Serializable]
        [LiveClass("TestNonShadowHost")]
        public class TestNonShadowHost
        {
            [SerializeField, LiveField]    // No [Hide] -> not a shadow
            [FormerlyNamedAs("value")]
            public int _value;
        ...
        [Serializable]
        [LiveClass("TestInheritedShadowDerived")]
        ...
        public void Shadow_PropertyInheritsFieldPersistability()
        {
            ...
            // The Shadow Field's [LiveField] defaults to persistable=true.
            // That persistable flag must propagate to the paired Property entry.
            Assert.IsTrue(entry.isPersistable,
                "Property should inherit isPersistable=true from the shadow field's [LiveField].");
        }
        ...
        public void Shadow_FindProperty_ByPropertyName_ReturnsProperty()
        {
            ...
            var liveObj = new LiveObjectHandle("test-shadow-1", LiveClass.Get<TestHost>(), target);
        ...
        public void Shadow_FindProperty_ByFieldName_ResolvesToPropertyViaFormerNames()
        {
            ...
            var target = new TestHost();
            var liveObj = new LiveObjectHandle("test-shadow-2", LiveClass.Get<TestHost>(), target);
            var prop = liveObj.FindProperty("_value");
        ...
        {
            // [LiveField] without [Hide] is NOT a shadow even with [FormerlyNamedAs("value")].
            // Both Property and Field should remain as independent propertyTypes entries.
            var ec = LiveClass.Get<TestNonShadowHost>();
            var entries = ec.propertyTypes;
        ...
        public void Shadow_FromJsonProperty_FiresPropertySetter()
        {
            ...
            var target = new TestHost();
            var liveObj = new LiveObjectHandle("test-shadow-3", LiveClass.Get<TestHost>(), target);
            var prop = liveObj.FindProperty("value");
            Assert.IsNotNull(prop);

            var p = prop.Value;
            LivePropertySerializer.FromJson("{\"value\": 42}", in p, _resolver);
        ...
        public void Shadow_FullFromJson_BypassesPropertySetter()
        {
            ...
            var target = new TestHost();
            var liveObj = new LiveObjectHandle("test-shadow-4", LiveClass.Get<TestHost>(), target);

            LivePropertySerializer.FromJson("{\"value\": 99}", liveObj, _resolver);
        ...
        public void Shadow_FullFromJson_LoadsLegacyFieldNameKey()
        {
            ...
            var target = new TestHost();
            var liveObj = new LiveObjectHandle("test-shadow-5", LiveClass.Get<TestHost>(), target);

            LivePropertySerializer.FromJson("{\"_value\": 77}", liveObj, _resolver);
        ...
        public void Shadow_Serialize_WritesPropertyNameAsJsonKey()
        {
            ...
            var liveObj = new LiveObjectHandle("test-shadow-6", LiveClass.Get<TestHost>(), target);

            var json = LivePropertySerializer.ToJson(liveObj, _resolver);
        ...
        #region Inherited shadow field (regression: LiveGameObject.name / .active)
        ...
        public void InheritedShadow_DerivedClass_DetectsBasePrivateShadowField()
        {
            ...
            // private fields. LiveGameObject.name / .active hit this path
            // (declared on LiveUnityObjectProxy / LiveGameObject) and were silently
            // dropped from scene.json persistence.
            var ec = LiveClass.Get<TestInheritedShadowDerived>();
            var entries = ec.propertyTypes;
        ...
        {
            // End-to-end: serializing the derived LiveObjectHandle for persistence
            ...
            var liveObj = new LiveObjectHandle("test-inherited-shadow-1",
                LiveClass.Get<TestInheritedShadowDerived>(), target);

            var json = LivePropertySerializer.ToJson(liveObj, _resolver, isDirtyOnly: false, forPersistence: true);
            var parsed = JObject.Parse(json);

            Assert.IsNotNull(parsed["value"],
                "Persistence output must include the shadow-backed property; " +
                "this is the regression that hid LiveGameObject.name and .active from scene.json.");
