...
{
    /// <summary>
    /// Verifies that a property path can address an array element by its [LiveKey] value
    /// ("entries[Beta].weight") instead of by index, and that the binding follows the keyed element
    /// across reordering. The key→index resolution happens in the LiveProperty graph layer, so the
    /// resolved property is index-based and get/set behave exactly like an index path.
    /// </summary>
    [TestFixture]
    public class LiveKeyPathTests
    {
        [Serializable]
        [LiveClass("KeyedEntry")]
        ...
        {
            LiveClass.Clear();
            LiveClass.RegisterFromAttributes<KeyedEntry>();
            LiveClass.RegisterFromAttributes<KeyedContainer>();
            ...
            _handle = LiveObjectRegistry.Create(_instance, "keyed_container").Value;
        }
