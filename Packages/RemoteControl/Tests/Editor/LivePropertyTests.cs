...
namespace Lilium.RemoteControl.Tests
{
    ...
    public class LivePropertyTests
    {
        ...
        private LiveObjectHandle _testObject;
        private TestClass _testInstance;
        private LiveObjectHandle _arrayObject;
        private TestArrayClass _arrayInstance;
        private LiveObjectHandle _structObject;
        private TestStructClass _structInstance;
        private LiveObjectHandle _enumerableObject;
        ...
        public void Setup()
        {
            ...
            // TestClass の登録
            var testDefines = new LivePropertyDefine[]
            {
                ...
                new LivePropertyDefine { name = "vectorValue", path = "vectorValue" },
                new LivePropertyDefine { name = "colorValue", path = "colorValue" },
                new LivePropertyDefine { name = "intField", path = "intField" },
                new LivePropertyDefine { name = "stringField", path = "stringField" }
            };
            LiveClass.Register<TestClass>("TestClass", testDefines);

            // TestArrayClass の登録
            var arrayDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "intArray", path = "intArray" },
                new LivePropertyDefine { name = "intList", path = "intList" },
                new LivePropertyDefine { name = "stringList", path = "stringList" },
                new LivePropertyDefine { name = "vectorList", path = "vectorList" }
            };
            LiveClass.Register<TestArrayClass>("TestArrayClass", arrayDefines);

            // TestEnumerableClass の登録
            var enumerableDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "stringEnumerable", path = "stringEnumerable" },
                new LivePropertyDefine { name = "intEnumerable", path = "intEnumerable" }
            };
            LiveClass.Register<TestEnumerableClass>("TestEnumerableClass", enumerableDefines);

            // NestedClass の登録
            var nested2Defines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "value", path = "value" }
            };
            LiveClass.Register<NestedClass2>("NestedClass2", nested2Defines);

            var nestedDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "value", path = "value" },
                new LivePropertyDefine { name = "array", path = "array" }
            };
            LiveClass.Register<NestedClass>("NestedClass", nestedDefines);
            ...
            _testObject = LiveObjectRegistry.Create(_testInstance, "test_object").Value;
            ...
            _arrayObject = LiveObjectRegistry.Create(_arrayInstance, "array_object").Value;

            _enumerableInstance = new TestEnumerableClass(new[] { "alpha", "beta", "gamma" });
            _enumerableObject = LiveObjectRegistry.Create(_enumerableInstance, "enumerable_object").Value;

            // TestStruct の登録
            var structDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "intValue", path = "intValue" },
                new LivePropertyDefine { name = "floatValue", path = "floatValue" },
                new LivePropertyDefine { name = "boolValue", path = "boolValue" }
            };
            LiveClass.Register<TestStruct>("TestStruct", structDefines);

            // TestStructClass の登録
            var structClassDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "structProperty", path = "structProperty" },
                new LivePropertyDefine { name = "structField", path = "structField" }
            };
            LiveClass.Register<TestStructClass>("TestStructClass", structClassDefines);
            ...
            _structObject = LiveObjectRegistry.Create(_structInstance, "struct_object").Value;
        }
        ...
        public void IsValid_ReturnsCorrectValue()
        {
            ...
            var invalidProperty = new LiveProperty(null, default, null);
        ...
        {
            var elementType = LivePropertyUtility.GetCollectionElementType(typeof(IEnumerable<string>));
            Assert.AreEqual(typeof(string), elementType);

            var intElementType = LivePropertyUtility.GetCollectionElementType(typeof(IEnumerable<int>));
        ...
        {
            Assert.IsTrue(LivePropertyUtility.IsArrayType(typeof(int[])));
            Assert.IsTrue(LivePropertyUtility.IsArrayType(typeof(List<string>)));
            Assert.IsTrue(LivePropertyUtility.IsArrayType(typeof(IEnumerable<string>)));
            Assert.IsFalse(LivePropertyUtility.IsArrayType(typeof(string)));
            Assert.IsFalse(LivePropertyUtility.IsArrayType(typeof(int)));
            Assert.IsFalse(LivePropertyUtility.IsArrayType(null));
        }
        ...
        public void FindProperty_NestedClassArray_WorksCorrectly()
        {
            ...
            var nestedObject = LiveObjectRegistry.Create(nestedInstance, "nested_object").Value;
        ...
        /// 要素が登録済みLiveObjectであればownerが正しく切り替わることを検証
        /// </summary>
        [Test]
        public void GetProperty_ObjectArrayElement_SwitchesOwnerToRegisteredLiveObject()
        {
            // TargetClass を登録
            var targetDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "health", path = "health" },
                new LivePropertyDefine { name = "label", path = "label" },
                new LivePropertyDefine { name = "scores", path = "scores" },
            };
            LiveClass.Register<TargetClass>("TargetClass", targetDefines);

            // SelectorClass を登録（objectsプロパティ）
            var selectorDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "objects", path = "objects" },
            };
            LiveClass.Register<SelectorClass>("SelectorClass", selectorDefines);
            ...
            var targetLiveObject = LiveObjectRegistry.Create<TargetClass>(targetInstance, "target_001");
            ...
            var selectorLiveObject = LiveObjectRegistry.Create<SelectorClass>(selectorInstance, "selector_001").Value;

            // セレクタパス経由でプロパティを取得: objects[0].health
            var healthProp = selectorLiveObject.FindProperty("objects[0].health");
            Assert.IsNotNull(healthProp);
            Assert.AreEqual(100, healthProp.Value.GetValue());

            // ownerがターゲットのLiveObjectに切り替わっていることを確認
            Assert.AreEqual(targetLiveObject, healthProp.Value.owner,
                "owner should be the target's LiveObjectHandle, not the selector's");
            Assert.AreNotEqual(selectorLiveObject, healthProp.Value.owner);
        ...
        public void SetValue_ObjectArrayElement_FiresOnPropertyChangingOnTargetType()
        {
            ...
            // インスタンスとLiveObjectを作成
            var targetInstance = new TargetClass { health = 100, label = "Player", scores = new List<int>() };
            var targetLiveObject = LiveObjectRegistry.Create<TargetClass>(targetInstance, "target_002");

            var selectorInstance = new SelectorClass { objects = new object[] { targetInstance } };
            var selectorLiveObject = LiveObjectRegistry.Create<SelectorClass>(selectorInstance, "selector_002").Value;

            // イベント監視用の変数
            LiveProperty? changingProperty = null;
            ...
            LiveClass.Get<TargetClass>().onPropertyChanging += (prop, newVal) =>
            {
            ...
            LiveClass.Get<SelectorClass>().onPropertyChanging += (prop, newVal) =>
            {
        ...
        /// ownerがセレクタのLiveObjectのままであることを検証
        /// </summary>
        [Test]
        public void GetProperty_ObjectArrayElement_KeepsOwnerWhenNotRegistered()
        {
            // TargetClass を登録（LiveClassのみ、LiveObjectは作成しない）
            var targetDefines = new LivePropertyDefine[]
            {
                new LivePropertyDefine { name = "health", path = "health" },
                new LivePropertyDefine { name = "label", path = "label" },
            ...
            // ターゲットインスタンスはLiveObjectとして登録しない
            var targetInstance = new TargetClass { health = 50, label = "NPC", scores = new List<int>() };

            var selectorInstance = new SelectorClass { objects = new object[] { targetInstance } };
            var selectorLiveObject = LiveObjectRegistry.Create<SelectorClass>(selectorInstance, "selector_003").Value;
            ...
            // ownerがセレクタのまま
            Assert.AreEqual(selectorLiveObject, healthProp.Value.owner,
                "owner should remain the selector when target is not a registered LiveObjectHandle");
        }
