...
{
    /// <summary>
    /// LiveObjectHandle 同士の親子関係の動作検証。
    /// - parentId は Unity hierarchy から派生する getter-only プロパティ
    /// - 親子関係の変更は LiveObjectRegistry.SetParent 経由 (内部で GameObjectUtility.SetTransformParent)
    /// - @parent シリアライズは desired Unity hierarchy の復元ポイント
    /// </summary>
    [TestFixture]
    public class LiveObjectParentTests
    {
        ...
        public void SetUp()
        {
            ...
            LiveClass.RegisterFromAttributes<TransformRef>();
        ...
        {
            var exposed = new LiveGameObject(_CreateGO("no-parent"));
            exposed.OnEnable();
        ...
        public void ParentId_Getter_FollowsUnityTransformHierarchy()
        {
            ...
            var parent = new LiveGameObject(parentGO);
            var child = new LiveGameObject(childGO);
            parent.OnEnable();
            child.OnEnable();
        ...
        [Test]
        public void ParentId_Getter_SkipsNonLiveAncestor()
        {
            ...
            var grand = new LiveGameObject(grandGO);
            var child = new LiveGameObject(childGO);
            grand.OnEnable();
            child.OnEnable();

            // middle は LiveObjectHandle ではないので透過して grand が親として返る
            Assert.AreEqual(grand.id, child.parentId);
        }

        // ------------------------------------------------------------
        // LiveObjectRegistry.SetParent / GetChildren / 循環検出
        // ------------------------------------------------------------
        ...
        public void SetParent_ValidParentAndChild_ReparentsTransformAndUpdatesGetter()
        {
            ...
            var ok = LiveObjectRegistry.SetParent(child.id, parent.id, out var err);
        ...
        {
            var a = new LiveGameObject(_CreateGO("a"));
            a.OnEnable();

            var ok = LiveObjectRegistry.SetParent(a.id, a.id, out var err);
        ...
        {
            var a = new LiveGameObject(_CreateGO("a"));
            var b = new LiveGameObject(_CreateGO("b"));
            a.OnEnable();
            b.OnEnable();

            // a -> b の親子関係を確立 (a が b 配下)
            Assert.IsTrue(LiveObjectRegistry.SetParent(a.id, b.id, out _));

            // b -> a は循環になるので reject
            var ok = LiveObjectRegistry.SetParent(b.id, a.id, out var err);
        ...
        public void SetParent_ClearParent_AcceptsNull()
        {
            ...
            parent.OnEnable();
            child.OnEnable();
            LiveObjectRegistry.SetParent(child.id, parent.id, out _);

            var ok = LiveObjectRegistry.SetParent(child.id, null, out var err);
        ...
        {
            var child = new LiveGameObject(_CreateGO("c"));
            child.OnEnable();

            var ok = LiveObjectRegistry.SetParent(child.id, "no-such-id", out var err);
        ...
        {
            var grand = new LiveGameObject(_CreateGO("grand"));
            var parent = new LiveGameObject(_CreateGO("p"));
            var child1 = new LiveGameObject(_CreateGO("c1"));
            var child2 = new LiveGameObject(_CreateGO("c2"));
            grand.OnEnable(); parent.OnEnable(); child1.OnEnable(); child2.OnEnable();

            LiveObjectRegistry.SetParent(parent.id, grand.id, out _);
            LiveObjectRegistry.SetParent(child1.id, parent.id, out _);
            LiveObjectRegistry.SetParent(child2.id, parent.id, out _);

            var children = LiveObjectRegistry.GetChildren(parent.id).ToList();
        ...
        {
            var root = new LiveGameObject(_CreateGO("root"));
            var child = new LiveGameObject(_CreateGO("child"));
            root.OnEnable(); child.OnEnable();
            LiveObjectRegistry.SetParent(child.id, root.id, out _);

            var roots = LiveObjectRegistry.GetRootObjects().Select(o => o.id).ToList();
        ...
        [Test]
        public void Serialize_LiveObjectWithParent_EmitsAtParentMetadata()
        {
            var parent = new LiveGameObject(_CreateGO("parent-s"));
            var child = new LiveGameObject(_CreateGO("child-s"));
            parent.OnEnable(); child.OnEnable();
            LiveObjectRegistry.SetParent(child.id, parent.id, out _);

            var json = LivePropertySerializer.ToJson(
                child.liveObject.Value, DefaultLiveObjectResolver.Instance);
        ...
        [Test]
        public void Serialize_LiveObjectWithoutParent_OmitsAtParent()
        {
            var child = new LiveGameObject(_CreateGO("child-no-parent"));
            child.OnEnable();
        ...
        public void Deserialize_JsonWithAtParent_RestoresUnityHierarchy()
        {
            ...
            {
                ["@type"] = child.liveObject.Value.targetTypeName,
                ["@id"] = child.id,
                ["@parent"] = parent.id,
            }.ToString();

            LivePropertySerializer.FromJson(json, child.liveObject.Value);
        ...
        {
            // name に対応する LiveObjectHandle が Registry に未登録なら null。
            // 起動シーケンス中の「親がまだ登録されていない」状態をシミュレート。
            _CreateGO("Main Avatar"); // LiveGameObject を作らない = Registry に未登録
        ...
        public void TransformRef_InitFromTransform_StoresRelativePath()
        {
            ...
            var rootLive = new LiveGameObject(rootGO);
            rootLive.OnEnable();
        ...
        {
            // Play mode 突入時、親 LiveObjectHandle がまだ registry に未登録の状態で Attach が走るケース。
        ...
        // ------------------------------------------------------------
        // LiveGameObjectWithTransform の hierarchy 同期
        // ------------------------------------------------------------

        [Test]
        public void WithTransform_UserDrag_SyncsTransformRef()
        {
            // LiveGameObjectWithTransform は自前で hierarchy change を購読し、
            ...
            var parentLive = new LiveGameObject(parentGO);
            var childLive = new LiveGameObjectWithTransform(childGO);
            parentLive.OnEnable();
            childLive.OnEnable();
            ...
            Assert.AreSame(parentGO, childLive.parent.ResolveOwner(),
