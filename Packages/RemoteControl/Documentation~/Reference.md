...
API reference for the `[Live*]` attributes and the REST endpoints served by `RemoteControlServerManager`.
...
|---|---|---|
| `[LiveClass]` | class / struct | Marks a type as remotely exposable. Required on any type whose members are meant to surface in the remote client. |
| `[LiveProperty]` | property / field | Surfaces the member for remote get/set. |
| `[LiveFunction]` | method | Allows the method to be invoked remotely. `icon` adds a Material Icons glyph to the button. |
| `[LiveEnum]` | enum | Publishes the enum's type definition so the remote client can render a dropdown. |
| `[Help("text")]` | any | Attaches help / description text. The string is treated as a localization key — see [Localization.md](Localization.md). |
...
| `[LiveDefault]` | static property | Provides a custom default value for a struct (used for `Reset` semantics). |
| `LivePropertyRef` | field type | A `readonly struct` that aliases an `LiveProperty` declared on another component. Useful for aggregation pages that surface properties from multiple components in one place. Value, dirty state, and revert all delegate to the referenced property. See [LiveObjectSpec.md](LiveObjectSpec.md) for details. |
...
| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/live/objects` | List all exposed objects (depth 1; add `?nested` for full expansion). |
| `GET` | `/live/objects?type={typeName}` | Filter exposed objects by type. |
| `GET` | `/live/object/{id}` | Fetch a single exposed object by id (depth 1; add `?nested` for full expansion). |
| `GET` | `/live/object/{id}/{path}` | Read a property value (always fully expanded). |
...
client polls `/live/changes` to learn what to refetch — so nobody receives data for a page they are
...
via `GET /live/object/{id}/{path}`. Arrays do not consume depth (element count and type stay
