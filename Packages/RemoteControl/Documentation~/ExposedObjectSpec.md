...
- `[ExposedField]` — 永続化される
- `[ExposedProperty]` — 永続化されない。永続化したい場合は次のいずれかを使う:
  - **Shadow Field パターン**: `[ExposedField, Hide, FormerlyExposedAs("propName")]` を持つ private field とペアにし、Property は値の get/set ラッパ + 副作用 (`Apply*`) として実装。永続化は Field 経由
