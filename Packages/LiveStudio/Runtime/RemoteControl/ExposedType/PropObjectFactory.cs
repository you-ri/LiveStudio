    ...
    public class PropObjectFactory : StandardObjectFactory
    {
        ...
        private async Task _CreatePropInstanceAsync(AssetBase asset)
        {
            ...
            if (_container == null)
            {
                ...
                if (_container == null)
                {
                    Debug.LogError("[LiveStudio] PropObjectFactory: ExposedObjectContainer not found.");
                    return;
                }
            }
            ...
            ExposedGameObject wrapper = prefab.GetComponent<IProp>() != null
                ? new ExposedGameObject(instance)
                : new ExposedGameObjectWithTransform(instance);
            wrapper.prefabSourceKey = instanceable.instanceKey;
            wrapper.name = _GenerateUniqueName(prefab.name);
            _container.AddExposedObject(wrapper);
            wrapper.OnEnable();
        }
