...
{
    [Serializable]
    public class ExposedCameraFactory : IExposedObjectFactory
    {
        [SerializeField]
        ...
        public IExposedObject Create()
        {
            if (prefab == null)
            {
                Debug.LogError("[Studio] ExposedCameraFactory.Create: prefab is null.");
                return null;
            }
            ...
            if (instance == null)
            {
                Debug.LogError($"[Studio] ExposedCameraFactory.Create: failed to instantiate prefab '{prefab.name}'.");
                return null;
            }
            ...
            if (camera == null)
            {
                Debug.LogError($"[Studio] ExposedCameraFactory.Create: prefab '{prefab.name}' has no CinemachineCamera component.");
                Lilium.RemoteControl.GameObjectUtility.DestroyWithUndo(instance);
                return null;
            }

            var exposed = new ExposedCamera(camera);
        ...
        public void RegisterPrefabs()
        {
            ...
            {
                Debug.LogWarning($"[Studio] ExposedCameraFactory.RegisterPrefabs: prefab '{prefab.name}' has no guid. Open the containing asset in Inspector to trigger OnValidate.");
                return;
            }
