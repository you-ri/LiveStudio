...
namespace Lilium.LiveStudio
{
    ...
    /// feature-specific switch actions. Created from the remote app's "bind to key" affordance next to a
    ...
    /// manager, so it commits once on key-up (<see cref="ActionContext.triggered"/>).
    ///
    ...
    /// from <see cref="ActionSet.enabled"/> (the user's on/off): the set can be enabled yet dangling.
    /// </summary>
    [Serializable]
    [ExposedClass(Category = "Action", Icon = "bolt")]
    public class InvokeFunctionAction : ActionBase
    {
        ...
        /// (never serialized): the remote app surfaces it so a dangling action reads as invalid, separate
        /// from <see cref="ActionSet.enabled"/>.</summary>
        [ExposedProperty]
        public bool valid
        {
            get
