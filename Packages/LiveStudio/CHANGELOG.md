...
- **The live data page is one card carrying five controls, and the replay can be scrubbed and looped.** Recording a take and playing one back were two sections, which is one card each on a page that only ever holds this much; they are the same job from the operator's side, so `FrameRecorderPage` now declares a single *Recording / Playback* section, the way the Fusion page's recorder already read. What it holds is the transport and nothing else: the take number, the recording to play, `loop`, `seekProgress`, and one button each for record, replay and stop. `seekProgress` is a 0..1 slider over the take that jumps and carries on playing from there, reading through `FrameRecorderController.replayIndex` / `replayFrameCount` / `SeekReplay` (already there for the LiveData Sequencer window). A take with no tail index — one that was cut short — reports 0 and cannot be scrubbed. Stop is one button rather than a stop each, since the component already tears down a replay before it stops a recording. What left the page is still reachable: the status readouts (`isRecording`, `recordedFrames`, `recordedMegabytes`, `isReplaying`, `replayFrame`) are exposed on the recorder object itself, and `keyframeInterval` / `compress` are project settings in the project's settings file and the inspector — neither is something reached for between takes, and both crowded out the controls that are. The section's translation keys are renamed to `SECTION_FRAME_RECORDER_TITLE` / `_SUBTITLE`.
