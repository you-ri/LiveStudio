...
- **The asset catalog is no longer written to the live scene, and a prop is put out as a scene object like everything else.** `ExternalAssetManager` kept a persistence shadow of its catalog (`_persistedAssets`) holding every entry that was in use, which made the live scene carry a second, quieter answer to a question the show already answered: which avatar is out is `ExternalAvatarSource.selectedAvatar`, which stage is standing is `StageManager.activeSet` / `loadedSets`, and a prop that is out is the instance in the scene. For props there was no such answer, so the entry doubled as one — and as the mooring its saved overrides hung from, through an `objectId` the loaded wrapper was re-keyed to. A prop now reaches the stage only as a scene instance, spawned from the scene page's "+" and persisted by its `@prefab` (`IInstantiableProp.instanceKey`) like a camera or any other object, which is also what lets the same prop be out several times. The manager no longer implements the (de)serialize callbacks at all, so nothing about the catalog — which files this machine has on disk — reaches a saved scene. `AssetBase.state`, `AssetBase.objectId`'s persistence and `reloadsOnAvatarChange` go with it (a scene-level instance is not destroyed with the avatar, so nothing has to be reloaded onto the next one), replaced by `AssetBase.isLoadable` for a catalog whose entries are not placements. The prop page is a list of the prop files the project has, with no load toggle and no per-entry property pane; both moved to the scene page, where the instance is. **Breaking:** a scene saved by an earlier build no longer restores its avatar, stage or props — the entries it holds are not read back. A free-standing `*.glb` / `*.gltf` is put out as a *GLTF Model* object (its `path` picks the file), and a prop `*.preset.json` is no longer loadable at all; prop presets could not be created since *Save as Preset* was removed, and reading them was the last thing keeping the shadow alive.

- **"Save as Preset" is gone.** It wrote a named `*.preset.json` from whatever was loaded, and for avatars it had stopped working anyway — the id the remote app hands over names the avatar's GameObject wrapper, which the resolver compared against `AvatarController` and rejected. What it existed for is now automatic: an avatar's settings are kept in its own preset beside it (see above), written without anyone pressing anything. `ExternalAssetManager.SaveAsPreset` and the remote app's button, name dialog and its translations are removed; `AssetStateSnapshot.CaptureFullDelta` and `PropAsset.CaptureDeltaState`, which existed for it, go with them. Reading `*.preset.json` is untouched: presets already in a project still list and load as before.

- **An avatar's settings belong to the avatar, and are kept in a preset file of its own rather than in the live scene.** The expression config (input weight adjustments, blink sync, the neutral expression, the expression list), the mesh visibility overrides, the animator parameter overrides, the body override clip, the avatar layer and the lower-body lock are all about one model — *its* meshes, *its* animator, *its* blend shapes. Saved with the scene they were lost the moment another scene was opened, and carried over onto whichever avatar came next. These members of `AvatarController` are now `PersistScope.Custom`, so the live scene writes none of them, and the controller writes them instead into a preset (`*.preset.json`, the same format a named preset uses) holding the delta from the avatar's defaults. An avatar that is a file is answered by a file beside it under the same base name — `Avatars/hoge.vrm` by `Avatars/hoge.preset.json` — so copying an avatar and its settings elsewhere is moving two files that already look like a pair; a built-in avatar, which has no file of its own, is answered from `{project}/Settings/AvatarPresets/` keyed by its catalog GUID, as is the scene's own avatar (`default`) and an avatar file outside the project (whose folder is not ours to write into). Such a file is not an entry of its own in the project: it *is* the avatar beside it, and the crawl skips it by path so one avatar does not list twice. Like a deck file there is no save button — an edit is written half a second after the last one, what the controller holds is written for the outgoing avatar on every switch and when the controller is disabled, and an avatar left at its defaults writes no file at all (resetting everything removes it). Switching avatars now puts these members back to their defaults before applying the incoming avatar's file, where before the previous avatar's values simply stayed on the controller. A live scene saved by an earlier build still restores them, and the next write moves them into the avatar's own file; saving that scene again drops them from it. A named preset (*Save as Preset*) now carries both scopes, so it brings back the avatar's settings along with the placement the live scene holds. When a scene holds more than one `AvatarController` — an additively loaded set can bring one — only the controller avatar loads are routed to keeps a file: which avatar is out is answered once, about that one controller, so both keeping settings would have them write the same file and overwrite each other. The other leaves its members where its scene put them. Recording is unchanged: the members a take carried still declare their lane, so hiding a mesh or locking the lower body mid-show is still recorded.
...
- **What the studio does reaches a take.** Deck operations go through `FrameGate` the same way a remote write does (`OperationRequest` rebuilds the request a network call would have sent), so a key press is recorded like a tap in the remote app. Values that move continuously ride the state lane instead of writing an event per change: the free and orbital-follow cameras' yaw / pitch / position / distance / fov / screen position (their getters now read the live rig rather than a shadow copy synced only on save), a camera's `priority`, the environment light, `OperationSet`'s manual value while a slider is dragged, and `TransformRef`'s bone path. `ExternalAssetManager` holds a replay while an asset it needs is still loading. The avatar's pose is carried by `MotionSourceBase` (see Changed). Machine-local settings — the screen's output settings, render quality, the project manager's poll intervals — stay out of takes.

- **Command-line options for runs that must not touch the operator's own files.** `LaunchArgs` reads `-name value` options for what has to be settled before anything else runs: `-savedBase <dir>` moves where every saved file goes (`SavedPaths`); `-project <dir>` opens that project instead of the remembered one, and is never written back, so the next ordinary launch is unaffected; `-noRemoteApp` / `-noCompanionApp` stop the build from starting its own remote app / companion app (Fusion), so an external driver can be the only client. `StartupSceneSwitcher` now asks `ProjectManager.ResolveStartupProjectPath` instead of reading `PlayerPrefs` itself, so both startup paths agree on which project's base scene to load. These were added for the integration test harness; options the class does not know are ignored, so engine options can share the command line.

- TODO: `Contents/Prefabs/Capture Camera.prefab` (carrying a `CaptureCameraTracker`) was added with the `ScreenManager` change — describe what it is for.
...
- **The screen follows whichever camera the standing set brings, and `ScreenController` is gone — its work belongs to `ScreenManager`.** The output settings sat on a component on one camera in the bootstrap scene, which made that camera everybody's: a set could not bring its own framing, post-processing or background without fighting the one the app had already put on air. `ScreenManager` is a persistent object in the *Live Studio System* prefab, alongside `StageManager` and `EnvironmentLight`, and it holds the settings while *finding* the camera — the camera in the scene of the set that is standing, falling back to the bootstrap scene's. Resolution and full screen are re-applied once and never move, because they belong to the window this app owns; only the background and Spout follow the camera across a switch, and the camera being left behind has its target texture and sender taken back off it. Exactly one camera is left running: every other camera carrying a `CinemachineBrain` is switched off along with its audio listener, since otherwise two brains follow the same virtual cameras and draw the same picture twice over, and two enabled listeners is a warning and an undefined listening position. A camera without a brain — a render-texture feed, an overlay — is somebody else's and is left alone, and a camera on an inactive GameObject is read as the author having switched it off deliberately. The Spout stream is now published under a fixed name rather than the camera's, so a set switch no longer drops the source out of OBS. The first camera resolved still seeds the starting background, so a fresh project inherits what the scene author set up; **only** the first, or each set's authored background would overwrite the operator's saved setting the moment that set loaded. `ScreenController` itself stays as an empty `[Obsolete]` component so a scene or prefab still referencing it opens without a missing script — the reference resolves, the component does nothing, and it can be removed at leisure. It exposes nothing, since `Screen` as a live class is `ScreenManager`'s name now and two types claiming it would fight over the settings file. An existing project's `Screen.settings.json` carries over unchanged (see `jp.lilium.remotecontrol`), and the shipped `Screen` prefab is now a plain camera, so every scene referencing it follows without being touched.

- **Picking a stage on the stage page is a complete switch.** Selecting a set flagged it active and left every other loaded set standing, which is the multi-scene reading of the operation; from the operator's side, choosing a stage is replacing the stage. The page now calls `StageManager.SwitchToSet`, which unloads the others and loads the target on demand — the same path the deck's switch-stage operation has always taken. Holding several sets at once is unchanged and still reachable: the per-set load toggles are where they were, and `SetActiveSet` still moves only the active flag for callers that want it. `SwitchToSet` takes an id where `SwitchToSetByName` takes a name, because names are not unique — two bundles can carry the same one — and anything holding the entry itself should say which set it means.

- **The Camera, GLTF Model and External Avatar Controller prefabs are declared in the shipped `UnityBasicLiveClasses` asset, following the move in `jp.lilium.remotecontrol`.** The scene page's "+" offers the same three in the same order, at the same access levels; the camera page's "+" now offers the camera, which it could not before because the list lived on the scene page. A project of your own adds its prefabs to its own live class asset -- the shipped one stays read-only -- and the Studio UI definition no longer carries a factory list at all.
...
- **`FrameRecorderController` moved to `jp.lilium.remotecontrol` (`Lilium.RemoteControl`), and this package now only tells it where takes go.** Everything the component did was that package's — the gate, the recorder, the replayer, retaining the state and structure systems for the length of a take, putting the inventory back on a supplied frame — except one thing: that a take belongs to the open project, the same way scenes, decks and avatars do. That answer stays here as `FrameRecorderProject`, which installs itself as the recorder's `recordingFolderProvider` on both a domain reload and a play — pointing it at the project's `Recordings` folder — and takes a take's picture when recording begins. The recorder is no longer a component either (see `jp.lilium.remotecontrol`): it is a registered object with a fixed id, so the *Live Studio System* prefab no longer carries it, and its exposed name is still `FrameRecorder`. Breaking only for source naming `Lilium.LiveStudio.FrameRecorderController`; nothing has shipped it. `ResolveRecordingFolder` and its tests came along to `FrameRecorderProject`.

- **A light is an ordinary GameObject that happens to carry a `Light` component — `LiveLight` is gone.** The proxy class was a copy of `LiveGameObjectWithTransform` (the same `transform` property, the same `TransformRef` parent, the same hierarchy re-attach) with four `Light` properties bolted on, so a light no longer needs a class of its own: the scene entry is a plain `LiveGameObjectWithTransform`, and `enabled` / `color` / `intensity` / `shadows` come from a **live class asset shipped with this package** (`Runtime/Resources/Settings/UnityBasicLiveClasses.asset`), applied by `LiveStudioLiveClassRegistry` at load. Nothing else had to be built: a `LiveGameObject` already lists every component whose type has a registered live class, and the serializer already writes those as `@source` entries, so the values are saved and restored exactly as before. The declarations register globally rather than per `RemoteControlContainer` because an unregistered type is skipped by the serializer — wiring them per scene would silently drop a light's values in any scene that forgot the reference; assets that travel with a set bundle still go through the container list. Two things look different in the remote app: a light's own properties render one level in, inside the component pane, and `shadows` is the real `LightShadows` dropdown (registered through `[assembly: LiveExternalEnum]`) rather than the old on/off `shadow` toggle. Every scene shipped here was migrated in place and kept its light's live-object id, so `enabled`, `color`, `intensity` and the transform still restore from an existing live scene; `shadow` and the light's `parent` do not carry over, since neither name survives. `LiveLightFactory` goes with it — no UI definition referenced it and the "+" menu never offered a light.

- **Takes and snapshots are project assets side by side, on one Recordings page.** A snapshot is live data one frame long and a take is a run of it, but they lived in separate folders with separate pages. `RecordingAsset` lists every `*.live.bin` (and legacy `*.livedata`) in the project the way `SnapshotAsset` lists snapshots, so a take appears on the project page like any other asset — with its picture served by the generic `GET /live/asset/{key}/@image`, a `Play` button, and the ordinary delete. `RecordingManager` is what the remote app drives for takes: the transport — record, replay, one stop, `loop`, and `seekProgress`, a 0..1 slider that scrubs the take and pauses it while being dragged, since a running playhead and a drag fight over the same frame — plus the take list, in the same shape as `SnapshotManager`'s so one card can draw either. When recording starts, the live camera's view is written beside the take as its picture (`LiveCameraThumbnail`, shared with snapshots, triggered by `FrameRecorderController.onRecordingStarted`). The status readouts (`isRecording`, `recordedFrames`, `isReplaying`, …) are on the recorder object itself, and `keyframeInterval` / `compress` are project settings. ⚠ Snapshots are now written to the project's `Recordings` folder, next to the takes, instead of `Snapshots`. The snapshot list reads only that folder, so move existing `*.snapshot.json` / `*.snapshot.png` files across to keep them on the page (the project listing still finds them where they are).

- ⚠ **A motion source records and replays the pose it produces, and the pose is in muscle space.** `HumanoidPoseData` is now a humanoid pose in muscle space — `bodyPosition` (divided by the avatar's scale), `bodyRotation` and the 95 muscles (`kMuscleCount`), each with a presence value alongside — instead of per-bone rotations and a hip position. A bone rotation only means something against the rest pose it was authored on, so a take in that form could only replay on the model it was shot with; muscles are normalized per joint, so the same numbers describe the same pose on a different body. `MotionSourceBase` now owns putting the pose on the state lane and reading it back, so a source only has to sample: implement `TrySample(out AvatarAnimationData, out long sampledFrom)` and `frameSource`, and optionally `OnFrameHeadBegin` and `PlacementMatrix`. The placement is applied after the pose reaches the lane, so what is recorded is the unplaced pose and the reference point can still be edited during a replay — and a scene replaying a take needs no capture machinery at all. Breaking for every custom motion source and for code reading `pose.hipPosition` / `boneRotations` / `bonePresences`; a derived class that overrides `OnEnable` / `OnDisable` must now call the base. `AvatarPoseWriter` applies the muscle pose to avatars that drive themselves (`StandardAvatar`, `VRM0Avatar`), and `HumanMuscle` names the 95 muscles, so a recorded pose reads as joints rather than numbered floats.

- **The selected avatar and the standing stage are saved in the live scene and restored as soon as their assets turn up.** `ExternalAvatarSource.selectedAvatar` and `StageManager.activeSet` / `loadedSets` used to read the catalog's current state, which is empty right after a scene restores and before the asset crawl has finished. They now hold the operator's choice, saved to the scene and carried on the state lane by the same declaration, and reconcile against the catalog as it fills in. `AvatarController.bodyOverrideClipKey` is a single key for the untracked-body override clip (a baked clip or a pack member), so that override reaches a take as well.

- The screen's background type and color are project settings now, like the rest of its output settings (resolution, full screen, Spout): the compositing background is chosen per machine, and is not part of the stage being recorded.

- The Project Settings page is `Project/Lilium Live Studio` (was `Project/Live Studio`).
...
- **A built-in set renamed or moved in the editor takes effect on a scene that was saved with it loaded.** The de-dup keeps the restored entry (that is what carries its enabled state), so it also carried whatever name and path it was saved with: a renamed set kept the old name for the session, and one whose scene had moved kept a path that no longer loads. Its display fields are now re-read from the declaration as the catalog is injected.
